...
public class MultistreamClientA {
	
	//Path to the configuration files for the client setup
	public static final String FIRST_CLIENT_CONFIGURATION_FILE = "clientA.properties";
	//The application logger
	private static final Logger LOGGER = LoggerFactory.getLogger(MultistreamClientA.class);
	...
	public void startClient(String clientConfigurationFile) throws NumberFormatException, IOException, InterruptedException {
		...
		Properties firstClientProps = MultistreamProcessor.loadEntityConfiguration(clientConfigurationFile);
		if (firstClientProps == null){
		...
		String hostname = firstClientProps.getProperty("host");
		Integer port = Integer.valueOf(firstClientProps.getProperty("port"));
		String appTerminatorKeyword = firstClientProps.getProperty("appTerminator");
		String processingTerminatorKeyword = firstClientProps.getProperty("processingTerminator");
		String bufferCapacity = firstClientProps.getProperty("bufferCapacity");
		...
		while (true){
			...
			if (StringUtils.isNotBlank(finalTestInput)){
				//Encode it and send it over the server via an output byte buffer
				byte[] finalMessage = finalTestInput.getBytes();
				ByteBuffer outputBuffer = ByteBuffer.wrap(finalMessage);
				multistreamClient.write(outputBuffer);
	...
	public static void main(String[] args){
		try {
			new MultistreamClientA().startClient(FIRST_CLIENT_CONFIGURATION_FILE);
